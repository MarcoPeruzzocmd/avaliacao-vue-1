...
const tarefasEmAberto = computed(() => {
    return tarefas.value.filter(tarefa => !tarefa.feita).length;
});
...
function reativarTarefa(id) {
    const index = tarefas.value.findIndex(tarefa => tarefa.id === id);
    if (index !== -1) {
        tarefas.value[index].feita = false;
    }
}
const turma = 2
...
<template>
    <section class="container">
        ...
        <AdicionarTarefa v-model="novaTarefa" @adicionarTarefa="adicionarTarefa" />
